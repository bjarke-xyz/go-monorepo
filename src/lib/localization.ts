import { DayPrices, FuelType, Price } from "./models";
...
function priceToKronerAndOrer(price: Price): { kroner: number; orer: number } {
  const parts = price.price.toString().split(".").map(Number);
  return {
    kroner: parts[0],
    orer: parts[1],
  };
